import React from 'react';
import Scroll from 'react-scroll';

const Element = Scroll.Element;
const NavTo  = Scroll.Link;

const pubData = [
  {
    title: 'Data and Electric Power',
    subTitle: 'From Deterministic Machines to Probabilistic Systems in Traditional Engineering',
    date: 'February 2016',
    url: 'http://www.oreilly.com/data/free/data-and-electric-power.csp',
    imageName: 'publication--data-and-electric-power.png',
  }, {
    title: 'National Space Weather Action Plan',
    subTitle: ' Federal implementation approach for the National Space Weather Strategy',
    date: 'October 2015',
    url: 'https://obamawhitehouse.archives.gov/sites/default/files/microsites/ostp/final_nationalspaceweatheractionplan_20151028.pdf',
    imageName: 'publication--final_nationalspaceweatheractionplan_20151028.png',
  }, {
    title: 'Considering TensorFlow for the Enterprise',
    subTitle: 'An Overview of the Deep Learning Ecosystem',
    date: 'November 2017',
    url: 'http://www.oreilly.com/data/free/considering-tensorflow-for-the-enterprise.csp',
    imageName: 'publication--considering-tensorflow-for-the-enterprise.gif',
  }, {
    title: 'Quadrennial Energy Review, Second Installment',
    subTitle: 'Transforming the Nation\'s Electricity System',
    date: 'January 2017',
    url: 'https://www.energy.gov/policy/initiatives/quadrennial-energy-review-qer',
    imageName: 'publication--quadrennial-energy-review.jpg',
  }
];

const Publication = (data) => (
  <div className='pure-u-lg-1-2 pure-u-1'>
    <a href={data.url} target="_blank">
      <div className='pub-container'>

        <div className='img-container'>
          <img className='pure-img' src={`./images/${data.imageName}`} />
        </div>

        <div className='pub-text'>
          <h3>{data.title}</h3>
          <p>{data.subTitle}</p>
          <div className='date-container'>
            <p><i className="far fa-calendar"></i>{data.date}</p>
          </div>
          <div className='more'>MORE</div>
        </div>

      </div>
    </a>
  </div>
);


const Publications = pubData.map(pubData => Publication(pubData));
export default class extends React.Component {
  displayName = 'Home-Resources';

  render() {
    return (
      <Element name="resources" className='section-resources _section pure-g'>

        <div className='pure-u-1-1 center-text'>
          <h4>Publications and Resources</h4>
          <hr/>
          <p>See the documents below for a sampling of some of our research and work.</p>
        </div>

        <div className='publications-container pure-u-1-1'>
          {Publications}
        </div>

      </Element>
    );
  }
};
